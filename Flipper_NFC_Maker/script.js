...
Pages read: ${this.getNfcPageCount()}`;
        ...
        getCCLength() {
            ...
            if (this.nfcDeviceType in ccLengths) {
                ...
                this.helper.throwException('Error: 无效的设备类型');
            }
        }
        ...
        NDEF_WiFi(data) {
            ...
            if (!params.SSID || !params.PASSWORD || !params.AUTH) {
                throw new Error('无效的 Wi-Fi 配置数据.');
            }
            ...
            for (let i = 0; i < ndefMessage.length; i++) {
                ...
                if (byteIndex === 4) {
                    ...
                    if (pageIndex >= this.nfcPages.length) {
                        throw new Error('NDEF 占用字节大于标签字节');
                    }
                }
            }
    ...
    async function sendToFlipper() {
        ...
        try {
            ...
            console.error('无法发送到Flipperzero:', error);
            statusDiv.textContent = `Error: ${error.message}. 请重试！`;
